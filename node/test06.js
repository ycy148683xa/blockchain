...
//转化为绝对路径
console.log('resolve: ' + path.resolve('test06.js'));
//路径中文件的后缀名
console.log('ext name: ' + path.extname('test06.js'));

//3，net模块  底层网络通讯，提供服务端和客户端的操作
//方法
...
//创建连接到 path 的 unix socket 。参数 connectListener 将会作为监听器添加到 'connect' 事件上。
//返回 'net.Socket'。
//7	net.createConnection(path[, connectListener])
//创建连接到 path 的 unix socket 。参数 connectListener 将会作为监听器添加到 'connect' 事件。
//返回 'net.Socket'。
//8	net.isIP(input)
...
//监听指定端口 port 和 主机 host ac连接。 默认情况下 host 接受任何 IPv4 地址(INADDR_ANY)的直接连接。
//端口 port 为 0 时，则会分配一个随机端口。
...
//他们可以在用户创建客户端(使用 connect())时使用, 或者由 Node 创建它们，
...
//8	close
//当 socket 完全关闭时触发。
//参数 had_error 是布尔值，它表示是否因为传输错误导致 socket 关闭。
//属性
...
//网络连接绑定的本地接口 远程客户端正在连接的本地 IP 地址，字符串表示。
...
//指定端口 port 和 主机 host，创建 socket 连接 。参数 host 默认为 localhost。
//通常情况不需要使用 net.createConnection 打开 socket。只有你实现了自己的 socket 时才会用到。
//3	socket.connect(path[, connectListener])
//打开指定路径的 unix socket。通常情况不需要使用 net.createConnection 打开 socket。
//只有你实现了自己的 socket 时才会用到。
...
//禁用纳格（Nagle）算法。默认情况下 TCP 连接使用纳格算法，在发送前他们会缓冲数据。
//将 noDelay 设置为 true 将会在调用 socket.write() 时立即发送数据。noDelay 默认值为 true。
//12	socket.setKeepAlive([enable][, initialDelay])
//禁用/启用长连接功能，并在发送第一个在闲置 socket 上的长连接 probe 之前，可选地设定初始延时。
//默认为 false。 设定 initialDelay （毫秒），来设定收到的最后一个数据包和第一个长连接probe之间的延时。
//将 initialDelay 设为0，将会保留默认（或者之前）的值。默认值为0.
//13	socket.address()
//操作系统返回绑定的地址，协议族名和服务器端口。
//返回的对象有 3 个属性，比如{ port: 12346, family: 'IPv4', address: '127.0.0.1' }。
//14	socket.unref()
//如果这是事件系统中唯一一个活动的服务器，调用 unref 将允许程序退出。
//如果服务器已被 unref，则再次调用 unref 并不会产生影响。
//15	socket.ref()
//与 unref 相反，如果这是唯一的服务器，在之前被 unref 了的服务器上调用 ref 将不会让程序退出（默认行为）。
//如果服务器已经被 ref，则再次调用 ref 并不会产生影响。
