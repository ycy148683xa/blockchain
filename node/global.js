//JavaScript有一个特殊的对象，成为全局对象。
//浏览器JavaScript中，window是全局对象
//Node.js中，global是全局对象，所有的全局变量都是global的属性。
//满足一下条件是全局变量：
//1, 在最外层定义的变量；Node.js中不可能在最外层定义变量。
//2，全局对象的属性；
//3，隐式定义的变量(即未定义直接赋值的变量)。
//注意：永远使用var 定义变量以避免引入全局变量，因为全局变量会污染
//命名空间，提高代码的耦合风险。

//__filename：正在执行的脚本文件名，输出绝对路径
console.log(__filename);

//__dirname: 正在执行的脚本所在的目录。
console.log(__dirname);

//setTimeout(cb, ms)：全局函数，在指定的毫秒数后执行指定函数  
//只执行一次指定函数。
//返回一个代表定时器的句柄值。
function printHello() {
...
//clearTimeout(t)：全局函数，停止一个之前通过setTimeout()创建的定时器。
//参数t是通过setTimeout()创建的定时器。
var t = setTimeout(printHello, 2000);
clearTimeout(t)

//setInterval(cb, ms)，全局函数，在指定的毫秒数后执行指定函数。
//返回一个代表定时器的句柄值。
//可以使用clearInterval(t)函数来清楚定时器。
//setInterval()方法会不停的调用函数，
//直到clearInterval()被调用或窗口被关闭。
//setInterval(printHello, 2000)

//console，用于控制台标准输出(stdout, stderr)。
//1	console.log([data][, ...])
//向标准输出流打印字符并以换行符结束。
//该方法接收若干个参数，
...
// 输出到终端
process.stdout.write("Hello World222222!" + "\n");
...
//注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)。
//8	setuid(id)
//设置进程的用户标识（参见setuid(2)）。接收数字 ID或字符串名字。
